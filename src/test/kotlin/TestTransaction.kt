...
class TestTransaction {
    ...
    fun `execute insert should give amount of changed rows`() {
        val result = q4k.transaction {
            execute("INSERT INTO test_table (test) VALUES :test", mapOf("test" to "Hello world!"))
        }
    ...
    fun `execute insert should not work for unknown tables`() {
        shouldThrowAny {
            q4k.transaction { execute("INSERT INTO unknown_table (test) VALUES ('test')") }
        }
    }
    ...
    fun `executeAndReturnAutoGeneratedKeys insert should give auto-generated keys for single row`() {
        val result = q4k.transaction {
            executeAndReturnAutoGeneratedKeys("INSERT INTO test_table (test) VALUES :test", mapOf("test" to "Hello world!"))
        }
        result[0] shouldHaveKey "id"
    ...
    fun `executeGetKey should give key for single insert`() {
        val result = q4k.transaction {
            executeGetKey<Long>("INSERT INTO test_table (test) VALUES :test", "id", mapOf("test" to "Hello world!"))
        }
        result shouldBeEqual 1L
    }
    ...
    fun `executeGetKey should error on invalid key`() {
        shouldThrowWithMessage<IllegalArgumentException>("There is no auto-generated key 'invalidKey' associated with this table") {
            q4k.transaction {
                executeGetKey<Long>("INSERT INTO test_table (test) VALUES :test", "invalidKey", mapOf("test" to "Hello world!"))
            }
        }
    }
    ...
    fun `executeGetKey should error on invalid mapping`() {
        shouldThrowWithMessage<IllegalArgumentException>("Key 'id' cannot be mapped to class kotlin.String") {
            q4k.transaction {
                executeGetKey<String>("INSERT INTO test_table (test) VALUES :test", "id", mapOf("test" to "Hello world!"))
            }
        }
    }
    ...
    fun `query results should be empty if nothing exists`() {
        val result = q4k.transaction {
            query<TestTable>("SELECT * FROM test_table")
        }
    ...
    fun `query results should contain one element if only one exists`() {
        insertRows(10)
        val result = q4k.transaction {
            query<TestTable>("SELECT * FROM test_table WHERE id=:id", mapOf("id" to 3L))
        }
    ...
    fun `queryFirst should only get the first result from multiple matching rows`() {
        insertRows(25)
        val result = q4k.transaction {
            queryFirst<TestTable>("SELECT * FROM test_table WHERE id>= :id", mapOf("id" to 15L))
        }
    ...
    fun `queryFirst should be null if no results are found`() {
        val result = q4k.transaction {
            queryFirst<TestTable>("SELECT * FROM test_table")
        }
        result.shouldBeNull()
    }
    ...
    fun `queryOnly should succeed if only one result exists`() {
        insertRows(1)
        val result: Either<QueryOnlyException, TestTable> = q4k.transaction {
            queryOnly<TestTable>("SELECT * FROM test_table")
        }
